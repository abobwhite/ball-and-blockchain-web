    ...
    return this.executeContract('getPolicies', []).then(result => {
      return result[0].map((item, index) => {
        return new Policy({
          ...
          ratingExpiration: moment().add(this.Web3.raw.toDecimal(result[2][index]), 'd').toDate(),
          offerExpiration: moment().add(this.Web3.raw.toDecimal(result[3][index]), 'd').toDate(),
          territoryOfIssue: this.Web3.raw.toAscii(result[4][index]),
